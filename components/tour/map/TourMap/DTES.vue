<template>
  <div class="tour-map-container" @click.self="$emit('click-container')">
    ...
    >
      ...
      <!-- Map Dots -->
      <TourMapDots :videos="videos" :interactive="interactive" region="dtes" />
    </svg>
  </div>
</template>
...
import TourMapDots from "./Dots.vue";
export default {
  components: { TourMap, TourMapDots },
  props: {
    interactive: {
      type: Boolean,
      required: false,
  ...
  methods: {},
  computed: {
    ...mapGetters(["videos", "userProgress"]),
  },
  mounted() {},
};
</script>
